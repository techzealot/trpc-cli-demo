...
### Q: 开发模式下（tsx/pnpm dev）能用补全吗

可以，但需要额外处理。`tsx`/`pnpm dev` 运行时 `process.argv[1]` 是脚本绝对路径，生成的补全脚本命令名为空。需要手动修正：

```bash
# 定义函数
trpc-cli-demo() {
    pnpm --dir /path/to/project dev -- "$@"
}

# eval 时用 sed 修正 compdef
eval "$(trpc-cli-demo complete zsh | sed 's/^_() {/_trpc_cli_demo() {/' | sed 's/^compdef _$/compdef _trpc_cli_demo trpc-cli-demo/')"
```

> 开发模式每次 Tab 会启动 Node + tsx，有明显延迟（1-2 秒），仅适合验证逻辑，日常使用请走编译流程。
